...
data.name);self.options.beforeSend(data,xhr);xhr.send(data)};self.reset=function(){self.resetQueue();self.resetPreview();self.resetDropzone();self.resetInput();_debug("Reset")};self.resetDropzone=function(){_initDragDrop(true);_debug("Dropzone Reset")};self.resetInput=function(){if(self.fileInput!==null)self.fileInput.replaceWith(clonedInput);self.fileInput=$("input.upload-input");_debug("Input Reset")};self.resetQueue=function(){self.queue.length=0;if(self.options.queue)self.options.queue.html("");
_debug("Queue reset");_debug("Queue: ");_debug(self.queue)};self.resetPreview=function(){if(self.options.preview)self.options.preview.html("");_debug("Preview Cleared")};self.getQueue=function(){_debug("Queue: ");_debug(self.queue)};self.send=function(){if(self.queue.length<=0){_debug("No files to upload");return}self.supportsFileUpload&&!self.options.forceIframe?_sendFileAjax():_sendFileIframe()};_init()}})(jQuery,window,document);